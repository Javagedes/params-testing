...
///
/// Every `Param::Access` is an `AccessList` (a `Cons`/`Nil` chain). Making
/// concatenation a total GAT on this trait is what lets the tuple `Param` impls
/// splice their members' lists together with no `where` bounds: the `Concat`
/// output is itself an `AccessList`, so folds nest freely. The `Nil` base case
/// is where "continue on to the rest of the params" happens — reaching the end
/// of the left list grafts the entire right list on.
...
/// Builds a type-level access list terminated by [`Nil`] from `Read`/`Write`
/// markers.
///
/// ```ignore
